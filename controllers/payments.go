				...
				if v.Operator == "HUBTEL" {
					...
					if status, err := models.GetStatusByName("PENDING"); err == nil {
						...
						if _, err := models.AddPayment_history(&payment_history); err == nil {
							...
							resp = responses.SendMoneyResponseDTO{StatusCode: 807, Result: nil, StatusDesc: "Send money error! " + err.Error()}
						}
					} else {
						logs.Error("Unable to get status PENDING: %v", err)
						resp = responses.SendMoneyResponseDTO{StatusCode: 808, Result: nil, StatusDesc: "Send money error! " + err.Error()}
					}
				}

			} else {
				logs.Error("Unable to get network ", err.Error())
				resp = responses.SendMoneyResponseDTO{StatusCode: 806, Result: nil, StatusDesc: "Send money error! " + err.Error()}
			}
		} else {
			logs.Error("Unable to find payment ", err.Error())
			resp = responses.SendMoneyResponseDTO{StatusCode: 805, Result: nil, StatusDesc: "Send money error! " + err.Error()}
		}
	} else {
		logs.Error("Unable to find corporate info ", err.Error())
		resp = responses.SendMoneyResponseDTO{StatusCode: 809, Result: nil, StatusDesc: "Send money error! " + err.Error()}
	}
	c.Data["json"] = resp
	c.ServeJSON()
}
